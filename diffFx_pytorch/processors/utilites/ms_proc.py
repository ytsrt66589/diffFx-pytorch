...
class MidSideProc(nn.Module):
    ...
    ----------
    proc : nn.Module
        The processor module to be applied to both mid and side channels.
        This processor should accept norm_params and dsp_params as arguments.
    
    Forward Parameters
    ----------------
    x : torch.Tensor
        Input stereo signal of shape [batch_size, 2, num_samples].
    mult : float, optional
        Multiplier for the mid/side conversion. Defaults to 0.5.
    norm_mid_params : Dict[str, torch.Tensor], optional
        Normalization parameters for mid channel.
    norm_side_params : Dict[str, torch.Tensor], optional
        Normalization parameters for side channel.
    dsp_mid_params : Dict[str, torch.Tensor], optional
        DSP parameters for mid channel processing.
    dsp_side_params : Dict[str, torch.Tensor], optional
        DSP parameters for side channel processing.
    
    Returns
    -------
    torch.Tensor
        Processed stereo signal of the same shape as input.
