...
class SendProc(nn.Module):
    ...
    ----------
    proc : nn.Module
        The processor module to be applied in parallel.
        This processor should accept norm_params and dsp_params as arguments.
    
    Forward Parameters
    ----------------
    x : torch.Tensor
        Input signal of shape [batch_size, channels, num_samples].
    mult : float, optional
        Mixing multiplier for the processed signal. Defaults to 0.5.
    norm_params : Dict[str, torch.Tensor], optional
        Normalization parameters for the processor.
    dsp_params : Dict[str, torch.Tensor], optional
        DSP parameters for the processor.
    
    Returns
    -------
    torch.Tensor
        Mixed signal of the same shape as input.
